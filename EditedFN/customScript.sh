...
function printHelp() {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"pubchannel\")"
  ...
  echo
  echo "	byfn.sh generate -c pubchannel"
  echo "	byfn.sh up -c pubchannel -s couchdb"
  echo "        byfn.sh up -c pubchannel -s couchdb -i 1.4.0"
  echo "	byfn.sh up -l node"
  echo "	byfn.sh down -c pubchannel"
  echo "        byfn.sh upgrade -c pubchannel"
  echo
...
function networkDown() {
  ...
  # stop kafka and zookeeper containers in case we're running with kafka consensus-typedocker-compose -f docker-compose-cli.yaml down --volumes --remove-orphan 
  docker-compose -f $COMPOSE_FILE -f $COMPOSE_FILE_COUCH -f $COMPOSE_FILE_KAFKA -f $COMPOSE_FILE_RAFT2 -f $COMPOSE_FILE_CA -f $COMPOSE_FILE_ORG3 down --volumes --remove-orphan
...
function generateChannelArtifacts() {
  ...
  if [ "$CONSENSUS_TYPE" == "solo" ]; then
    ...
    configtxgen -profile SampleDevModeKafka -channelID sys-channel -outputBlock ./channel-artifacts/genesis.block
  elif [ "$CONSENSUS_TYPE" == "etcdraft" ]; then
...
CLI_DELAY=3
# channel name defaults to "pubchannel"
